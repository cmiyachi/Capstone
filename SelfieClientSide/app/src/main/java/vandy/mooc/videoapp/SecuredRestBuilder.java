 ...
 * handle an OAuth 2.0 password grant loginEditText flow. The RestAdapter that it produces uses an interceptor
...
public class SecuredRestBuilder extends RestAdapter.Builder {
	...
	private class OAuthHandler implements RequestInterceptor {
		...
		public void intercept(RequestFacade request) {
			// If we're not logged in, loginEditText and store the authentication token.
			if (!loggedIn) {
				try {
